// rv32_ooo_int_iq.sv — Integer Issue Queue
// 8 entries, 2-source wakeup, multi-completion snooping (Int, Load, and FP)
...
(
  ...
  always_comb begin
    ...
    for (int i = 0; i < INT_IQ_ENTRIES; i++) begin
      if (entries[i].valid) num_valid = num_valid + 4'd1;
    end
  end

  assign dispatch_ready = (num_valid < INT_IQ_ENTRIES[3:0]) && (core_state == CORE_RUN);
  ...
  iq_entry_t next_entries [INT_IQ_ENTRIES-1:0];

  always_comb begin
    logic [2:0] insert_pos;
    if (issue_valid && issue_ready) begin
      insert_pos = 3'(num_valid - 4'd1);
    end else begin
      insert_pos = 3'(num_valid);
    end

    for (int i = 0; i < INT_IQ_ENTRIES; i++) begin
      // Step 1: Compaction candidate
      iq_entry_t candidate;
      if (issue_valid && issue_ready && i >= selected_idx) begin
        if (i < INT_IQ_ENTRIES - 1) begin
      ...
      if (dispatch_valid && dispatch_ready && 3'(i) == insert_pos) begin
