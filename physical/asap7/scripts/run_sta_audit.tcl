...
puts $audit_fp "Flow: OpenROAD Flow Scripts / OpenSTA (26Q1-2900-gdf79404cd8)"
puts $audit_fp "PDK: ASAP7 7.5T RVT (Predictive 7nm FinFET)"
puts $audit_fp ""
...
foreach clk [all_clocks] {
    ...
    puts $audit_fp "Waveform:         0.0 [expr $cperiod / 2.0] ps (50% duty cycle)"
}
puts $audit_fp ""
...
puts $audit_fp "Clock Transition:  20.0 ps"
puts $audit_fp ""

puts $audit_fp "--------------------------------------------------------------------------------"
puts $audit_fp "3. I/O DELAYS & CONSTRAINTS"
puts $audit_fp "--------------------------------------------------------------------------------"
...
puts $audit_fp "Input Delay (nominal):          200.0 ps (20% of 1.0 ns clock)"
puts $audit_fp "Output Delay (nominal):         200.0 ps (20% of 1.0 ns clock)"
puts $audit_fp "Input Transition:               20.0 ps"
puts $audit_fp "Output Load:                    2.0 fF"
puts $audit_fp ""

puts $audit_fp "--------------------------------------------------------------------------------"
puts $audit_fp "4. EXCEPTION CONSTRAINTS"
puts $audit_fp "--------------------------------------------------------------------------------"
puts $audit_fp "False Paths:     0 (none defined; all paths strictly synchronous)"
puts $audit_fp "Multicycle Paths: 0 (none defined; all single-cycle 1-period setup / 0-period hold)"
puts $audit_fp ""
...
puts $audit_fp "Constrained Endpoints Rate:     100.0%"
puts $audit_fp "================================================================================"
close $audit_fp
puts "Wrote constraint_audit.txt"

# 2. Path Class Reports
puts "Generating timing_reg2reg.rpt..."
...
puts "Generating timing_in2reg.rpt..."
...
puts "Generating timing_reg2out.rpt..."
...
puts "Generating timing_in2out.rpt..."
...
puts "Path class reports completed."
exit
